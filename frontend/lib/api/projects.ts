...
export const projectsApi = {
  ...
  // Agreement flow
  sendAgreement: async (projectId: string, agreementMessage: string, agreementFile?: File): Promise<Project> => {
    try {
      const formData = new FormData();
      formData.append('agreement_message', agreementMessage);
      if (agreementFile) formData.append('agreement_file', agreementFile);

      const response = await fetch(`${API_BASE_URL}/projects/${projectId}/agreement/send`, {
        method: 'POST',
        headers: (() => {
          const token = getCookie('auth_token');
          const headers: Record<string, string> = { 'Accept': 'application/json' };
  ...
  clientAcceptAgreement: async (projectId: string, acceptMessage: string, signedFile?: File): Promise<Project> => {
    try {
      const formData = new FormData();
      formData.append('accept_message', acceptMessage);
      if (signedFile) formData.append('signed_agreement_file', signedFile);

      const response = await fetch(`${API_BASE_URL}/projects/${projectId}/agreement/client-accept`, {
        method: 'POST',
        headers: (() => {
          const token = getCookie('auth_token');
          const headers: Record<string, string> = { 'Accept': 'application/json' };
  ...
  clientRejectAgreement: async (projectId: string, feedbackMessage: string): Promise<Project> => {
    try {
      const response = await fetch(`${API_BASE_URL}/projects/${projectId}/agreement/client-reject`, {
        method: 'POST',
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || 'Failed to reject agreement');
      }
      const data = await response.json();
      return projectsApi._mapApiProject(data);
    } catch (error) {
  ...
  managerReviewAgreement: async (projectId: string, action: 'accept' | 'reject', feedbackMessage?: string): Promise<Project> => {
    try {
      const response = await fetch(`${API_BASE_URL}/projects/${projectId}/agreement/manager-review`, {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ action, feedback_message: feedbackMessage || '' }),
      });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || 'Failed to review agreement');
      }
      const data = await response.json();
      return projectsApi._mapApiProject(data);
    } catch (error) {
